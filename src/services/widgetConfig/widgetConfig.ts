...
import { EVENT_MESSAGES } from '../../server/const';
...
export const widgetConfigLoader = (widgetDir: string): Promise<FileLoaderResponse> => new Promise((resolve, reject) => {
    fs.readFile(`${widgetDir}/${WidgetFileType.WIDGET_CONFIGURATION}`, 'utf8', (configLoadError: Error, data: string) => {
        if (!data) {
            fs.readFile(`${widgetDir}/${WidgetFileType.WIDGET_SCHEMA}`, 'utf8', (schemaLoadError: Error, schemaData: string) => {
